...
<input type="hidden" name="courses[]" value="{{$request->courses[0]}}">
    ...
    <div class="item">
        ...
        <input type="text" placeholder="せい"  name="fSei" class="half_txt" pattern="^[ぁ-ん]{1,30}+$?^[ァ-ヶ]{1,30}+$">　<input type="text"
            placeholder="めい"  name="fMei" class="half_txt" pattern="^[ぁ-ん]{1,30}+$|^[ァ-ヶ]{1,30}+$">
    </div>
    <div class="item">
        <p class="sub_title">　電話番号(ハイフン無し)　　<span class="must">必須</span></p>
        <input type="text" placeholder=""  name="tel" class="txt" pattern="^[0-9]{8,14}+$" required>
    </div>
   ...
   <div id="number-form" class="payjs-outer"></div>
    ...
    <div><span class="btn" id="create_tkn">　照会する　</span><span id="token2" class="alert"></span></div>
    @endif
